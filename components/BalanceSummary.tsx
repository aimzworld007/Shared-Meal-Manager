 ...
 * @summary Displays a table summarizing the financial status of each participant.
 */
import React from 'react';

/**
 * Represents the calculated summary for a single participant.
 */
interface ParticipantSummary {
    ...
    name: string;
    /** The total amount deposited by the participant. */
    totalDeposit: number;
    /** The participant's equal share of the total expenses. */
    share: number;
...
interface BalanceSummaryProps {
    /** An array of calculated participant summaries. */
    summaries: ParticipantSummary[];
}

/**
 * Renders a table showing each participant's deposits, share of expenses, and final balance.
 * @param {BalanceSummaryProps} props - The component props.
 * @param {ParticipantSummary[]} props.summaries - The list of participant financial summaries to display.
                        ...
                        {summaries.length === 0 ? (
                             <tr>
                                <td colSpan={4} className="px-6 py-4 text-center text-sm text-gray-500">
                                    Add participants to see a balance summary.
                                </td>
                            </tr>
                        ) : (
                            summaries.map((s) => (
